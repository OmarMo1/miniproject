...
def spoor(vertrektijd):
    ''' Deze functie leest het treinplanner en geeft de spoor als return '''
    with open('treinplanner.xml', 'r') as treinplannerFile:
        content = xmltodict.parse(treinplannerFile.read())
        for line in content['ReisMogelijkheden']['ReisMogelijkheid']:
...
def aankomst(vertrektijd):
    ''' Deze functie leest het treinplanner en geeft de aankomst als return '''
    with open('treinplanner.xml', 'r') as treinplannerFile:
        content = xmltodict.parse(treinplannerFile.read())
        for line in content['ReisMogelijkheden']['ReisMogelijkheid']:
            if vertrektijd in line['ActueleVertrekTijd'][11:19]:
...
def reistijd(vertrektijd):
    ''' Deze functie leest het treinplanner en geeft de reistijd als return '''
    with open('treinplanner.xml', 'r') as treinplannerFile:
        content = xmltodict.parse(treinplannerFile.read())
        for line in content['ReisMogelijkheden']['ReisMogelijkheid']:
            if vertrektijd in line['ActueleVertrekTijd'][11:19]:
...
def storing():
    '''
    Hier wordt de storingen.xml gelezen.
    De data in de storing.xml zit er html tags in.
    De return data wordt doorgestuurd naar striphtml.
    '''
    with open('storingen.xml', 'r') as storingenFile:
        ...
        if content['Storingen']['Ongepland']:
            try:
                ...
                data = content['Storingen']['Ongepland']['Storing']['Bericht'] #als de html tag [ongepland] niet is sturen we deze data.
                return data

        else:
            data = 'Er zijn geen ongeplande storingen'  #als de html tag [ongepland] niet is sturen we deze data.
            return data
...
def striphtml(data):
    ''' Hier wordt de de gelezen CDATA van de functie storing gelezen en alle html tags verwijderd '''
    p = re.compile(r'<.*?>')
    return p.sub('', data) #Het html gestripte data wordt gereturned
